...
function Checkout() {
    ...
    const handleOrder = async (e) => {
        ...
        if (cookies.get('token') && cookies.get('info')) {
            if (billValues.name && billValues.address && billValues.phone) {
                const total = cart.reduce(
                    (accumulator, previousValue) => accumulator + previousValue.price * previousValue.quantity,
                    0,
                );
                await request
                    .post(config.apis.createBill, {
                        billValues,
                        cart,
                        total,
                    })
                ...
                setNotification('Please complete all information!!');
            }
        } else {
                ...
                {cart.length > 0 ? (
                    ...
                    <div className="text-center w-full">
                        ...
                        <Link to="/products" className="mt-3 block ">
                            <span className="text-primary italic">Continues shopping</span>
                        </Link>
                    </div>
                )}
            </div>
